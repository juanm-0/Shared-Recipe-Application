...
- Checking items off (`ShoppingListItem.is_checked` exists on the model but isn't exposed via the API). Checking off was deferred to get full coverage of the stated requirements first, but it is possible to edit quantities in the shopping list, or remove items from it
